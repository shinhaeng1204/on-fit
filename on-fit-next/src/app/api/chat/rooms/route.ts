...
export async function GET() {
  ...
  // 각 방의 마지막 메시지 + unread 계산에 필요한 필드까지 같이 가져오기
  const { data: messages, error: messagesError } = await supabase
    .from("messages")
    .select("room_id, content, created_at, sender_id, read_by") // ← 필드 추가
    .in("room_id", roomIds)
  ...
  // 참고: 지금은 "나" 기준으로 계산해서 최소 1명.
  // 실제 인원수를 알고 싶으면 별도 participants 쿼리 필요.
