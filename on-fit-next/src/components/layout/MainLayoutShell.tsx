...
export default function MainLayoutShell({
  ...
  const { notifications, markAllRead, deleteOne, markOneRead, deleteAll } =
    useNotifications()

  // 🔹 새로고침해도(같은 탭) 마지막 위치 기억하기: sessionStorage
  useEffect(() => {
    if (typeof window === 'undefined') return

    const savedRegion = window.sessionStorage.getItem('onfit.region')
    const savedCoords = window.sessionStorage.getItem('onfit.coords')
    ...
    if (savedCoords) {
      try {
        ...
        console.error('Invalid coords in sessionStorage', e)
      }
    }
  }, [])
      ...
      <div>
        ...
        <LocationModal
          ...
          onSelect={(label, lat, lng) => {
            ...
            // ✅ 새로고침해도(같은 탭) 유지되도록 sessionStorage에 저장
            if (typeof window !== 'undefined') {
              window.sessionStorage.setItem('onfit.region', label)
